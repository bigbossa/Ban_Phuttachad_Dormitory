...
export default function TenantDetailsDialog({
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <Badge variant="secondary">{tenant.residents || "ผู้เช่า"}</Badge>
            </div>
          </div>
  ...
  <div className="pt-4 border-t">
    <h4 className="text-sm font-semibold flex items-center gap-2 mb-3">
      <Users className="h-4 w-4" /> ลูกเช่าในห้องเดียวกัน
    </h4>
        ...
        return (
          <div key={roommate.id}>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="text-lg font-semibold">{fullName}</h3>
                <Badge variant="secondary">{roommate.residents || "ลูกเช่า"}</Badge>
              </div>
            </div>
              ...
              {roommate.current_room && (
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium">ห้องที่เช่า</p>
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant="outline">ห้อง {roommate.current_room.room_number}</Badge>
                      <span className="text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      <Building className="h-3 w-3 text-muted-foreground" />
                      <span className="text-xs text-muted-foreground">
                        ชั้น {roommate.current_room.floor}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="text-xs text-muted-foreground mt-4">
            <p>เข้าพักเมื่อ: {new Date(tenant.created_at!).toLocaleDateString("th-TH")}</p>
            {tenant.updated_at && (
